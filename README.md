...
An augmentation of the V8 JS compiler to explore support for JS dialects. \
These interpreted languages derived from JS in aggregate are currently called **jspeak**.

We already internationalize and localize applications written with JavaScript. \
Why not do the same for the JavaScript language itself also? _(Rhetorical question; don't answer that here..)_
...
```shell
git pull origin
# version chosen with `tools/probe_v8_sdk_versions.sh` for older mac sdk v13 compatibility
gclient sync --with_branch_heads --with_tags --revision v8-speak@branch-heads/11.3
```
...
3. Update all keys at all levels in `.gclient_entries`, `.gclient_previous_sync_commits`, `.gcs_entries` to match the directory name `v8-speak`.
...
# optionally add remote alias for v8 to be more explicit, like "root" or "v8"
git remote add root https://chromium.googlesource.com/v8/v8.git
```

# Compile

Use `tools/dev/gm.py`. See [v8.dev/docs/build-gn](https://v8.dev/docs/build-gn) for details.

```shell
python tools/dev/gm.py --help
```

The main executable target is `d8-speak`, which is the shell jspeak interpreter.
